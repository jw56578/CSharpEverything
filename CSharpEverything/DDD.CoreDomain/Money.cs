...
namespace DDD.CoreDomain
{
    ...
    /// There is no ID
    /// you would not persist this thing into the database
    /// it is only used at runtime to organize information
    /// one value type can be equal to another if there are referring to the same object in memeory
    /// they can also be equal if they are structurally equivelant, all field/properties values match
    /// this object is also Immutable, you cannot change the values of an existing one, you can just make a new one
    /// This object cannot live by itself, some Entity must use this value object else it is pointless
    /// you would never create a Money object and then just send it to the UI to be shown, there would be no point of that
    /// </summary>
